...
3. Review the completed event strip, add optional text above the photos (a name, organization, hashtag, or blank), maximize any preview, and retake only the selected photo when needed.
4. Approve the set. The booth downloads one ZIP backup containing the three enhanced shots and the completed event strip.
5. Print the currently saved 1–4 groups at any time. The booth composes one 300 DPI A4 sheet using the supplied layout, leaving unused strip positions blank. Any strip can be removed before final printing.
